...
import { bulkDeleteStudents, bulkUpdateYearSem } from "@/lib/actions";
...
type Props = {
  ...
  /**
   * Server-rendered action buttons per row.
   * Pass these from the server page so async Server Components
   * (like FormContainer) stay outside this client component tree.
   *
   * Usage in page.tsx:
   *   actionMap={{ [item.id]: <><FormContainer .../><FormContainer .../></> }}
   */
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        setFeedback({
          type: "success",
          msg: `Deleted ${res.deleted} student(s).`,
        });
        clearSelection();
        router.refresh();
      } else {
          ...
          {selected.size > 0 && (
            <>
              ...
              <div className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-1.5">
                <RefreshCw className="h-3.5 w-3.5 flex-shrink-0 text-slate-400" />
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
              ...
              <th className="hidden lg:table-cell py-3 pr-4">Year / Sem</th>
            ...
            {data.map((item) => (
              ...
              >
                ...
                <td className="hidden lg:table-cell py-3 pr-4">
                  <span className="rounded-full bg-amber-50 border border-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700">
